...
module "management_resources" {
  ...
  custom_azure_backup_geo_codes                = lookup(local.management_resources_advanced, "custom_azure_backup_geo_codes", local.empty_map)
...
module "identity_resources" {
  ...
  # Optional input variables (basic configuration)
  location = coalesce(local.configure_identity_resources.location, local.default_location)
  tags     = local.identity_resources_tags

  # Optional input variables (advanced configuration)
  resource_prefix                  = lookup(local.identity_resources_advanced, "resource_prefix", local.empty_string)
  resource_suffix                  = lookup(local.identity_resources_advanced, "resource_suffix", local.empty_string)
  custom_azure_backup_geo_codes    = lookup(local.identity_resources_advanced, "custom_azure_backup_geo_codes", local.empty_map)
  custom_settings_by_resource_type = lookup(local.identity_resources_advanced, "custom_settings_by_resource_type", local.empty_map)
}
...
module "landingzones_resources" {
  ...
  # Mandatory input variables
  enabled             = local.deploy_landingzones_resources
  root_id             = local.root_id
  tenant_id           = local.tenant_id
  resource_type_names = local.resource_type_names
  subscription_id     = local.subscription_id_landingzones
  settings            = local.configure_landingzones_resources.settings

  # Optional input variables (basic configuration)
  location = coalesce(local.configure_landingzones_resources.location, local.default_location)
  tags     = local.landingzones_resources_tags

  # Optional input variables (advanced configuration)
  resource_prefix                  = lookup(local.landingzones_resources_advanced, "resource_prefix", local.empty_string)
  resource_suffix                  = lookup(local.landingzones_resources_advanced, "resource_suffix", local.empty_string)
  custom_azure_backup_geo_codes    = lookup(local.landingzones_resources_advanced, "custom_azure_backup_geo_codes", local.empty_map)
  custom_settings_by_resource_type = lookup(local.landingzones_resources_advanced, "custom_settings_by_resource_type", local.empty_map)
}
