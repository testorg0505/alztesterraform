...
# The following locals are used to build the map of Azure
# Expressroute Circuits to deploy.
locals {
  azurerm_express_route_circuit_virtual_wan = {
    for resource in module.connectivity_resources.configuration.azurerm_express_route_circuit :
    resource.resource_id => resource
    if resource.managed_by_module
  }
}
...
locals {
  azurerm_firewall_policy_rule_collection_group_virtual_wan = {
