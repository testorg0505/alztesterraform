...
locals {
  es_archetype_config_defaults = {
    ...
    "${local.root_id}-platform-connectivity" = {
    ...
    "${local.root_id}-platform-identity" = {
    ...
    "${local.root_id}-landingzones-online" = {
      archetype_id   = "es_online"
      parameters     = local.empty_map
      access_control = local.empty_map
    }
    "${local.root_id}-landingzones-sap" = {
      archetype_id   = "es_sap"
      parameters     = local.empty_map
      access_control = local.empty_map
    }
...
locals {
  subscription_ids_management = distinct(compact(concat(
    ...
    local.es_subscription_ids_map["${local.root_id}-platform-management"],
  )))
  subscription_ids_connectivity = [
    for id in distinct(compact(concat(
      [local.subscription_id_connectivity],
      local.es_subscription_ids_map["${local.root_id}-platform-connectivity"],
    ))) :
    id
    if !contains(local.subscription_ids_management, id)
  ]
  /*subscription_ids_landingzones = [
    for id in distinct(compact(concat(
      [local.subscription_id_landingzones],
      local.es_subscription_ids_map["${local.root_id}-landingzones"],
    ))) :
    id
    if !contains(local.subscription_ids_management, id)
  ]*/
  subscription_ids_identity = [
    for id in distinct(compact(concat(
      ...
      local.es_subscription_ids_map["${local.root_id}-platform-identity"],
    ))) :
    id
    if !contains(local.subscription_ids_management, id) &&
...
locals {
  ...
  es_core_landing_zones = {
    ...
    "${local.root_id}-landingzones" = {
      ...
      parent_management_group_id = local.root_id
      subscription_ids           = local.es_subscription_ids_map["${local.root_id}-landingzones"]
      archetype_config           = local.es_archetype_config_map["${local.root_id}-landingzones"]
    }
    "${local.root_id}-platform" = {
    ...
    "${local.root_id}-platform-connectivity" = {
      ...
      archetype_config           = local.es_archetype_config_map["${local.root_id}-platform-connectivity"]
    }
    "${local.root_id}-platform-management" = {
      display_name               = "Management"
      parent_management_group_id = "${local.root_id}-platform"
      subscription_ids           = local.subscription_ids_management
      archetype_config           = local.es_archetype_config_map["${local.root_id}-platform-management"]
    }
    "${local.root_id}-platform-identity" = {
  ...
  es_corp_landing_zones = {
    "${local.root_id}-landingzones-corp" = {
      ...
      subscription_ids           = local.es_subscription_ids_map["${local.root_id}-landingzones-corp"]
      archetype_config           = local.es_archetype_config_map["${local.root_id}-landingzones-corp"]
    }
  }
  es_online_landing_zones = {
    "${local.root_id}-landingzones-online" = {
      display_name               = "Online"
      parent_management_group_id = "${local.root_id}-landingzones"
      subscription_ids           = local.es_subscription_ids_map["${local.root_id}-landingzones-online"]
      archetype_config           = local.es_archetype_config_map["${local.root_id}-landingzones-online"]
    }
  }
  es_sap_landing_zones = {
    "${local.root_id}-landingzones-sap" = {
      display_name               = "SAP"
      parent_management_group_id = "${local.root_id}-landingzones"
      subscription_ids           = local.es_subscription_ids_map["${local.root_id}-landingzones-sap"]
      archetype_config           = local.es_archetype_config_map["${local.root_id}-landingzones-sap"]
    }
  }
