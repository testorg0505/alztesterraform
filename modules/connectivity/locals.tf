...
#Extended vWAN Hub configuration for Azure Bastion spoke
locals {
  ext_vwan_bastion_networks = local.settings.ext_vwan_bastion_networks
  # We generate the ext_vwan_bastion_networks_by_location as a map
  # to ensure the user has provided unique values for
  # each hub location. If duplicates are found,
  # terraform will throw an error at this point.
  ext_vwan_bastion_networks_by_location = {
    for ext_vwan_bastion_network in local.ext_vwan_bastion_networks :
    coalesce(ext_vwan_bastion_network.config.location, local.location) => ext_vwan_bastion_network
  }
  ext_vwan_bastion_network_locations = keys(local.ext_vwan_bastion_networks_by_location)
}

#Extended vWAN Hub configuration for Azure Private DNS Resolver
locals {
  ext_vwan_dns_networks = local.settings.ext_vwan_dnsresolver_networks
  # We generate the ext_vwan_dns_networks_by_location as a map
  # to ensure the user has provided unique values for
  # each hub location. If duplicates are found,
  # terraform will throw an error at this point.
  ext_vwan_dns_networks_by_location = {
    for ext_vwan_dns_network in local.ext_vwan_dns_networks :
    coalesce(ext_vwan_dns_network.config.location, local.location) => ext_vwan_dns_network
  }
  ext_vwan_dns_network_locations = keys(local.ext_vwan_dns_networks_by_location)
}
...
locals {
  ...
  deploy_ext_vwan_dns_network = {
    ...
    deploy_ext_private_dns_resolver = {
    for location, extend_vwan_dns_network in local.ext_vwan_dns_networks_by_location :
    location =>
    local.deploy_ext_vwan_dns_network[location] &&
    extend_vwan_dns_network.enabled
  }
  deploy_ext_private_dns_resolver_inbound_endpoint = {
    for location, extend_vwan_dns_network in local.ext_vwan_dns_networks_by_location :
    location =>
    local.deploy_ext_vwan_dns_network[location] &&
    local.deploy_ext_private_dns_resolver[location] &&
    extend_vwan_dns_network.config.deploy_private_dns_resolver_inbound_endpoint
  }
  deploy_ext_private_dns_resolver_outbound_endpoint = {
    for location, extend_vwan_dns_network in local.ext_vwan_dns_networks_by_location :
    location =>
    local.deploy_ext_vwan_dns_network[location] &&
    local.deploy_ext_private_dns_resolver[location] &&
    extend_vwan_dns_network.config.deploy_private_dns_resolver_outbound_endpoint
  }
  deploy_ext_private_dns_resolver_dns_forwarding_ruleset = {
    for location, extend_vwan_dns_network in local.ext_vwan_dns_networks_by_location :
    location =>
    local.deploy_ext_vwan_dns_network[location] &&
    local.deploy_ext_private_dns_resolver[location] &&
    local.deploy_ext_private_dns_resolver_outbound_endpoint[location] &&
    extend_vwan_dns_network.config.deploy_private_dns_resolver_dns_forwarding_ruleset
  }
}
...
locals {
  ...
  resource_group_names_by_scope_and_location = {
    connectivity = {
      ...
      "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-connectivity${local.resource_suffix}")
    }
    virtual_wan = {
      for location in local.virtual_wan_locations :
      location =>
    ...
    extended_vwan = {
      for location in local.ext_vwan_bastion_network_locations :
      location =>
      try(local.custom_settings.azurerm_resource_group["extended_vwan"][location].name,
      "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-bastion${local.resource_suffix}")
    }
    extended_vwan_dns = {
      for location in local.ext_vwan_bastion_network_locations :
      location =>
      try(local.custom_settings.azurerm_resource_group["extended_vwan_dns"][location].name,
      "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-dnsrs${local.resource_suffix}")
    }
    ddos = {
      (local.ddos_location) = try(local.custom_settings.azurerm_resource_group["ddos"][local.ddos_location].name,
      "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, local.dns_location, local.dns_location)}-${local.resource_prefix}-ddos${local.resource_suffix}")
    }
    dns = {
      (local.dns_location) = try(local.custom_settings.azurerm_resource_group["dns"][local.dns_location].name,
      "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, local.dns_location, local.dns_location)}-${local.resource_prefix}-dns${local.resource_suffix}")
    }
  }
...
locals {
  ...
  "${local.resource_type_names.azure_ddos}-${lookup(local.custom_azure_backup_geo_codes, local.ddos_location, local.ddos_location)}-${local.resource_prefix}-01${local.resource_suffix}")
...
locals {
  subnets_by_virtual_network = {
    ...
    local.virtual_network_resource_id[location] => concat(
      ...
      # Conditionally add Azure Bastion subnet
      local.deploy_bastion_host[location] ? [
        {
          # Resource logic attributes
          resource_id               = "${local.virtual_network_resource_id[location]}/subnets/AzureBastionSubnet"
          location                  = location
          network_security_group_id = null
          route_table_id            = null
          # Resource definition attributes
...
locals {
  bastion_host_name = {
    for location in local.hub_network_locations :
    location =>
    try(local.custom_settings.bastion_host["connectivity"][location].name,
    "${local.resource_type_names.azure_bastion}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  /*bastion_host_id = {
    for location in local.hub_network_locations :
    location =>
  ...
  bastion_host_prefix = {
    for location in local.hub_network_locations :
    location =>
    "${local.virtual_network_resource_group_id[location]}/providers/Microsoft.Network/bastionHosts"
  }*/
  bastion_host_resource_id = {
    for location in local.hub_network_locations :
    location =>
    "${local.virtual_network_resource_id_prefix[location]}/${local.virtual_network_name[location]}"
  }
  bastion_host_pip_name = {
    for location in local.hub_network_locations :
    location =>
    try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].name,
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.azure_bastion}-01")
  }
  bastion_host_pip_resource_id_prefix = {
    for location in local.hub_network_locations :
    location =>
      ...
      azurerm_public_ip     = (
        ...
        : concat(
          [
            {
              # Resource logic attributes
              resource_id       = local.bastion_host_pip_resource_id_prefix[location]
              managed_by_module = local.deploy_bastion_host[location]
              # Resource definition attributes
              name                    = local.bastion_host_pip_name[location]
              resource_group_name     = local.resource_group_names_by_scope_and_location["connectivity"][location]
              location                = location
              ip_version              = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].ip_version, null)
              idle_timeout_in_minutes = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].idle_timeout_in_minutes, null)
              domain_name_label       = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].domain_name_label, null)
              reverse_fqdn            = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].reverse_fqdn, null)
              public_ip_prefix_id     = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].public_ip_prefix_id, null)
              ip_tags                 = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].ip_tags, null)
              tags                    = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].tags, local.tags)
              sku                     = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].sku, "Standard")
              allocation_method       = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].allocation_method, "Static")
              zones                   = try(local.custom_settings.azurerm_public_ip["connectivity_bastion"][location].zones, local.bastion_host_pip_availability_zones[location])
            }
          ]
        )
      )
...
locals {
  er_gateway_name = {
    ...
    "${local.resource_type_names.express_route_gateway}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ...
  er_gateway_pip_name = {
    ...
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.express_route_gateway}-01")
  }
...
locals {
  vpn_gateway_name = {
    ...
    "${local.resource_type_names.vpn_gateway}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ...
  vpn_gateway_pip_name = {
    ...
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.vpn_gateway}-01")
  }
  vpn_gateway_pip_2_name = {
    for location in local.hub_network_locations :
    location =>
    try(local.custom_settings.azurerm_public_ip["connectivity_vpn_2"][location].name,
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.vpn_gateway}-02")
  }
...
locals {
  azfw_name = {
    ...
    "${local.resource_type_names.azure_firewall}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ...
  azfw_policy_name = {
    ...
    "${local.resource_type_names.azure_firewall_policy}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01")
  }
  ...
  azfw_pip_name = {
    ...
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.azure_firewall}-01")
  }
  azfw_mgmt_pip_name = {
    for location in local.hub_network_locations :
    location =>
    try(local.custom_settings.azurerm_public_ip["connectivity_firewall"][location].mgmt_name,
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.azure_firewall}-02")
  }
  ...
  virtual_hub_azfw_name = {
    ...
    "${local.resource_type_names.azure_firewall}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.vwan_hub}-01${local.resource_suffix}")
  }
  ...
  virtual_hub_azfw_policy_name = {
    ...
    "${local.resource_type_names.azure_firewall_policy}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.vwan_hub}-01")
  }
...
locals {
  virtual_wan_name = {
    ...
    "${local.resource_type_names.virtual_wan}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}${local.resource_suffix}")
  }
...
locals {
  virtual_hub_name = {
    ...
    "${local.resource_type_names.vwan_hub}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}${local.resource_suffix}")
  }
...
locals {
  virtual_hub_express_route_gateway_name = {
    ...
    "${local.resource_type_names.express_route_gateway}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.vwan_hub}-01${local.resource_suffix}")
  }
...
locals {
  virtual_hub_vpn_gateway_name = {
    ...
    "${local.resource_type_names.vpn_gateway}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.vwan_hub}-01${local.resource_suffix}")
  }
...
locals {
  azurerm_public_ip = flatten([
    for azurerm_public_ip in concat(
      ...
      #ext vwan
      local.azurerm_ext_vwan_bastion_host.*.azurerm_public_ip,
    ) :
    azurerm_public_ip
...
locals {
  ...
  #azurerm_virtual_network_peering_spoke_to_hub = flatten(
  #  [
  #    for location, hub_config in local.hub_networks_by_location :
  #    [
  #      for spoke_resource_id in hub_config.config.spoke_virtual_network_resource_ids :
  #      {
  #        # Resource logic attributes
  #        resource_id       = "${spoke_resource_id}/virtualNetworkPeerings/peering-${uuidv5("url", local.virtual_network_resource_id[location])}"
  #        managed_by_module = local.deploy_outbound_virtual_network_peering[location]
  #        # Resource definition attributes
  #        name                      = "peering-${uuidv5("url", local.virtual_network_resource_id[location])}"
  #        resource_group_name       = "${split("/", spoke_resource_id)[4]}"
  #        virtual_network_name      = "${split("/", spoke_resource_id)[8]}"
  #        remote_virtual_network_id = local.virtual_network_resource_id[location]
  #        # Optional definition attributes
  #        allow_virtual_network_access = true
  #        allow_forwarded_traffic      = true
  ...
  azurerm_virtual_network_peering = distinct(concat(
    ...
    #local.azurerm_virtual_network_peering_spoke_to_hub
  ))
}
...
locals {
  azurerm_virtual_hub_connection = flatten(
    [
      for location, virtual_hub_config in local.virtual_hubs_by_location :
      [
        for spoke_resource_id in distinct(concat(virtual_hub_config.config.spoke_virtual_network_resource_ids, virtual_hub_config.config.secure_spoke_virtual_network_resource_ids, [local.ext_vwan_bastion_network_resource_id[location]], [local.ext_vwan_dns_network_resource_id[location]])) :
        {
          # Resource logic attributes
...
#Azure vWAN Hub extension Bastion Network Configuration
locals {
  ext_vwan_bastion_network_name = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    try(local.custom_settings.azurerm_virtual_network["extended_vwan"][location].name,
    "${local.resource_type_names.virtual_network}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${replace(lookup(local.ext_vwan_bastion_networks_by_location, location, location).config.address_space[0],"/","_")}${local.resource_suffix}")
  }
  ext_vwan_bastion_network_resource_group_id = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    local.resource_group_config_by_scope_and_location["extended_vwan"][location].resource_id
  }
  ext_vwan_bastion_network_resource_id_prefix = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    "${local.ext_vwan_bastion_network_resource_group_id[location]}/providers/Microsoft.Network/virtualNetworks"
  }
  ext_vwan_bastion_network_resource_id = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    "${local.ext_vwan_bastion_network_resource_id_prefix[location]}/${local.ext_vwan_bastion_network_name[location]}"
  }
  ext_vwan_bastion_network = [
    for location, ext_hub_config in local.ext_vwan_bastion_networks_by_location:
    {
      # Resource logic attributes
      resource_id = local.ext_vwan_bastion_network_resource_id[location]
      # Resource definition attributes
      name                = local.ext_vwan_bastion_network_name[location]
      resource_group_name = local.resource_group_names_by_scope_and_location["extended_vwan"][location]
      address_space       = ext_hub_config.config.address_space
      location            = location
      bgp_community       =  null
      dns_servers         = ext_hub_config.config.dns_servers
      tags                = try(local.custom_settings.azurerm_virtual_network["extended_vwan"][location].tags, local.tags)
      ddos_protection_plan = ext_hub_config.config.link_to_ddos_protection_plan ? [
        {
          id     = local.ddos_protection_plan_resource_id
          enable = true
        }
...
#ext Bastion subnet configuration
locals {
  bastion_network_security_group_resource_id_prefix = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    "${local.ext_vwan_bastion_network_resource_group_id[location]}/providers/Microsoft.Network/networkSecurityGroups"
  }
  bastion_subnets_by_virtual_network = {
    for location, ext_vwan_bastion_network in local.ext_vwan_bastion_networks_by_location:
    local.ext_vwan_bastion_network_resource_id[location] => concat(
      # Get customer specified subnets and add additional required attributes
      [
        for subnet in ext_vwan_bastion_network.config.subnets : merge(
          subnet,
          {
            # Resource logic attributes
            resource_id                   = "${local.ext_vwan_bastion_network_resource_id[location]}/subnets/${subnet.name}"
            location                      = location
            network_security_group_id     = "${local.bastion_network_security_group_resource_id_prefix[location]}/${local.resource_type_names.network_security_group}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${subnet.name}${local.resource_suffix}"
            #route_table_id                = "${local.bastion_route_table_resource_id_prefix[location]}/${local.resource_type_names.route_table}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${subnet.name}${local.resource_suffix}"
            #routes                        = coalesce(subnet.routes, local.empty_list)
            rules                         = coalesce(subnet.rules, local.empty_list)
            # Resource definition attributes
            name                                          = subnet.name
            resource_group_name                           = local.resource_group_names_by_scope_and_location["extended_vwan"][location]
            virtual_network_name                          = local.ext_vwan_bastion_network_name[location]
            private_endpoint_network_policies     = try(local.custom_settings.azurerm_subnet["extended_vwan"][location][subnet.name].private_endpoint_network_policies, null)
            private_link_service_network_policies_enabled = try(local.custom_settings.azurerm_subnet["extended_vwan"][location][subnet.name].private_link_service_network_policies_enabled, null)
            service_endpoints                             = try(local.custom_settings.azurerm_subnet["extended_vwan"][location][subnet.name].service_endpoints, null)
            service_endpoint_policy_ids                   = try(local.custom_settings.azurerm_subnet["extended_vwan"][location][subnet.name].service_endpoint_policy_ids, null)
            delegation                                    = try(local.custom_settings.azurerm_subnet["extended_vwan"][location][subnet.name].delegation, local.empty_list)
          }
        )
      ]
    )
  }
  azurerm_ext_vwan_bastion_subnet = flatten([
    for subnets in local.bastion_subnets_by_virtual_network :
    subnets
  ])
}
...
locals {
  azurerm_ext_vwan_bastion_nsg = [
    for subnet in local.azurerm_ext_vwan_bastion_subnet : {
      # Resource logic attributes
      resource_id   = subnet.network_security_group_id
      subnet_id     = subnet.resource_id
      # Resource definition attributes
      name                = try(local.custom_settings.azurerm_network_security_groups["extended_vwan"][subnet.vnet_identifier][subnet.name].name, try(split("/", subnet.network_security_group_id)[8],""))
      resource_group_name = try(local.custom_settings.azurerm_network_security_groups["extended_vwan"][subnet.vnet_identifier][subnet.name].resource_group_name, subnet.resource_group_name)
      location            = try(local.custom_settings.azurerm_network_security_groups["extended_vwan"][subnet.vnet_identifier][subnet.name].location, subnet.location)
      tags                = try(local.custom_settings.azurerm_network_security_groups["extended_vwan"][subnet.vnet_identifier][subnet.name].tags, local.tags)
      security_rule       = try(local.custom_settings.azurerm_network_security_groups["extended_vwan"][subnet.vnet_identifier][subnet.name].security_rule, subnet.rules)
    }
  ]
}

# Configuration settings for resource type:
#  - azurerm_ext_vwan_subnet_network_security_group_association
locals {
...
locals {
  ext_vwan_bastion_host_name = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    try(local.custom_settings.bastion_host["extended"][location].name,
    "${local.resource_type_names.azure_bastion}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ext_vwan_bastion_host_resource_id = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    "${local.ext_vwan_bastion_network_resource_id_prefix[location]}/${local.ext_vwan_bastion_network_name[location]}"
  }
  ext_vwan_bastion_host_pip_name = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].name,
    "${local.resource_type_names.public_ip}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${local.resource_type_names.azure_bastion}-01")
  }
  ext_vwan_bastion_host_pip_resource_id_prefix = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    "${local.ext_vwan_bastion_network_resource_group_id[location]}/providers/Microsoft.Network/publicIPAddresses"
  }
  ext_vwan_bastion_host_pip_availability_zones = {
    for location, ext_hub_config in local.ext_vwan_bastion_networks_by_location :
    location =>
    #try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].zones,
    concat(ext_hub_config.config.availability_zones.zone_1 == true ? ["1"] : [],
      ext_hub_config.config.availability_zones.zone_2 == true ? ["2"] : [],
      ext_hub_config.config.availability_zones.zone_3 == true ? ["3"] : [],
    )
  ...
  azurerm_ext_vwan_bastion_host = [
    for location,  ext_hub_config in local.ext_vwan_bastion_networks_by_location :
    {
     ...
     # resource_id      = "${local.ext_vwan_bastion_network_resource_id_prefix[location]}/${local.ext_vwan_bastion_network_name[ext_vwan_bastion_network.config.location]}"
      ...
      resource_group_name   = local.resource_group_names_by_scope_and_location["extended_vwan"][location]
      location              = location
      sku                   = ext_hub_config.config.sku
      subnet_id             = try(local.custom_settings.bastion_host["extended_vwan"][location].ip_configuration.subnet_id, "${local.ext_vwan_bastion_network_resource_id[location]}/subnets/AzureBastionSubnet")
      public_ip_address_id  = try(local.custom_settings.bastion_host["extended_vwan"][location].ip_configuration.public_ip_address_id, "${local.ext_vwan_bastion_host_pip_resource_id_prefix[location]}/${local.ext_vwan_bastion_host_pip_name[location]}")
      tags                  = try(local.custom_settings.bastion_host["extended_vwan"][location].tags, local.tags)
      azurerm_public_ip     = (
        # The following logic ensures that no `azurerm_public_ip` is created by the module if a custom `ip_configuration` is provided
        length(try(local.custom_settings.bastion_host["extended_vwan"][location].ip_configuration, local.empty_map)) > 0
        ? local.empty_list
        : concat(
          [
            {
              ...
              resource_group_name     = local.resource_group_names_by_scope_and_location["extended_vwan"][location]
              location                = location
              ip_version              = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].ip_version, null)
              idle_timeout_in_minutes = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].idle_timeout_in_minutes, null)
              domain_name_label       = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].domain_name_label, null)
              reverse_fqdn            = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].reverse_fqdn, null)
              public_ip_prefix_id     = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].public_ip_prefix_id, null)
              ip_tags                 = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].ip_tags, null)
              tags                    = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].tags, local.tags)
              sku                     = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].sku, "Standard")
              allocation_method       = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].allocation_method, "Static")
              zones                   = try(local.custom_settings.azurerm_public_ip["extended_vwan_bastion"][location].zones, local.ext_vwan_bastion_host_pip_availability_zones[location])
            }
          ]
        )
      )
...
#Azure vWAN Hub extension Private DNS Resolver Network Configuration
locals {
  ext_vwan_dns_network_name = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    try(local.custom_settings.azurerm_virtual_network["extended_vwan_dns"][location].name,
    "${local.resource_type_names.virtual_network}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-${replace(lookup(local.ext_vwan_dns_networks_by_location, location, location).config.address_space[0],"/","_")}${local.resource_suffix}")
  }
  ext_vwan_dns_network_resource_group_id = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    local.resource_group_config_by_scope_and_location["extended_vwan_dns"][location].resource_id
  }
  ext_vwan_dns_network_resource_id_prefix = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/Microsoft.Network/virtualNetworks"
  }
  ext_vwan_dns_network_resource_id = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_id_prefix[location]}/${local.ext_vwan_dns_network_name[location]}"
  }
  ext_vwan_dns_network = [
    for location, ext_hub_config in local.ext_vwan_dns_networks_by_location:
    {
      # Resource logic attributes
      resource_id = local.ext_vwan_dns_network_resource_id[location]
      # Resource definition attributes
      name                = local.ext_vwan_dns_network_name[location]
      resource_group_name = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
      address_space       = ext_hub_config.config.address_space
      location            = location
      bgp_community       =  null
      dns_servers         = ext_hub_config.config.dns_servers
      tags                = try(local.custom_settings.azurerm_virtual_network["extended_vwan_dns"][location].tags, local.tags)
      ddos_protection_plan = ext_hub_config.config.link_to_ddos_protection_plan ? [
        {
          id     = local.ddos_protection_plan_resource_id
          enable = true
...
#extendedvwandns subnet
locals {
  /*dns_network_security_group_resource_id_prefix = {
    for location in local.ext_vwan_bastion_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/Microsoft.Network/networkSecurityGroups"
  }*/
   dns_subnets_by_virtual_network = {
    for location, ext_vwan_dns_network in local.ext_vwan_dns_networks_by_location:
    local.ext_vwan_dns_network_resource_id[location] => concat(
    local.deploy_ext_private_dns_resolver[location] ? [
        {
          # Resource logic attributes
          resource_id               = "${local.ext_vwan_dns_network_resource_id[location]}/subnets/PrivateResolverIn"
          location                  = location
          network_security_group_id = null
          route_table_id            = null
          #rules                         = coalesce(subnet.rules, local.empty_list)
          # Resource definition attributes
          name                                          = "PrivateResolverIn"
          address_prefixes                              = [ext_vwan_dns_network.config.address_prefix_in, ]
          resource_group_name                           = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
          virtual_network_name                          = local.ext_vwan_dns_network_name[location]
          private_endpoint_network_policies     = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverIn"].private_endpoint_network_policies, null)
          private_link_service_network_policies_enabled = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverIn"].private_link_service_network_policies_enabled, null)
          service_endpoints                             = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverIn"].service_endpoints, null)
          service_endpoint_policy_ids                   = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverIn"].service_endpoint_policy_ids, null)
          delegation                                    = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverIn"].delegation,
                                                          [{
                                                              name = "delegation"
                                                              service_delegation = [{
                                                                name = "Microsoft.Network/dnsResolvers"
      ...
      local.deploy_ext_private_dns_resolver[location] ? [
        {
          # Resource logic attributes
          resource_id               = "${local.ext_vwan_dns_network_resource_id[location]}/subnets/PrivateResolverOut"
          location                  = location
          network_security_group_id = null
          route_table_id            = null
          #rules                         = coalesce(subnet.rules, local.empty_list)
          # Resource definition attributes
          name                                          = "PrivateResolverOut"
          address_prefixes                              = [ext_vwan_dns_network.config.address_prefix_out, ]
          resource_group_name                           = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
          virtual_network_name                          = local.ext_vwan_dns_network_name[location]
          private_endpoint_network_policies     = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverOut"].private_endpoint_network_policies, null)
          private_link_service_network_policies_enabled = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverOut"].private_link_service_network_policies_enabled, null)
          service_endpoints                             = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverOut"].service_endpoints, null)
          service_endpoint_policy_ids                   = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverOut"].service_endpoint_policy_ids, null)
          delegation                                    = try(local.custom_settings.azurerm_subnet["extended_vwan_dns"][location]["PrivateResolverOut"].delegation,
                                                          [{
                                                              name = "delegation"
                                                              service_delegation = [{
                                                                name = "Microsoft.Network/dnsResolvers"
  ...
  azurerm_ext_vwan_dns_subnet = flatten([
    for subnets in local.dns_subnets_by_virtual_network :
    subnets
  ])
}

# Configuration settings for resource type:
#  - azurerm_ext_private_dns_resolver
locals {
  ext_private_dns_resolver_name = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].name,
    "${local.resource_type_names.private_dns_resolver}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ext_private_dns_resolver_prefix = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/Microsoft.Network/dnsResolvers"
  }
  ext_private_dns_resolver_resource_id = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_private_dns_resolver_prefix[location]}/${local.ext_private_dns_resolver_name[location]}"
  }
  ext_private_dns_resolver_inbound_endpoint_name = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    try(local.custom_settings.private_dns_resolver_inbound_endpoint["extended_vwan_dns"][location].name,
    "${local.resource_type_names.private_dns_resolver_inbound_endpoint}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ext_private_dns_resolver_inbound_endpoint_prefix = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/Microsoft.Network/dnsResolvers/${local.ext_private_dns_resolver_name[location]}/inboundEndpoints"
  }
  ext_private_dns_resolver_inbound_endpoint_resource_id = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_private_dns_resolver_inbound_endpoint_prefix[location]}/${local.ext_private_dns_resolver_inbound_endpoint_name[location]}"
  }
  ext_private_dns_resolver_outbound_endpoint_name = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    try(local.custom_settings.private_dns_resolver_outbound_endpoint["extended_vwan_dns"][location].name,
    "${local.resource_type_names.private_dns_resolver_outbound_endpoint}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
  ext_private_dns_resolver_outbound_endpoint_prefix = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/Microsoft.Network/dnsResolvers/${local.ext_private_dns_resolver_name[location]}/outboundEndpoints"
  }
  ext_private_dns_resolver_outbound_endpoint_resource_id = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_private_dns_resolver_outbound_endpoint_prefix[location]}/${local.ext_private_dns_resolver_outbound_endpoint_name[location]}"
  }
  ext_private_dns_resolver_dns_forwarding_ruleset_name = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    try(local.custom_settings.private_dns_resolver_dns_forwarding_ruleset["extended_vwan_dns"][location].name,
    "${local.resource_type_names.private_dns_resolver_dns_forwarding_ruleset}-${lookup(local.custom_azure_backup_geo_codes, location, location)}-${local.resource_prefix}-01${local.resource_suffix}")
  }
   ext_private_dns_resolver_dns_forwarding_ruleset_prefix = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/Microsoft.Network/dnsForwardingRulesets"
  }
  ext_private_dns_resolver_dns_forwarding_ruleset_resource_id = {
    for location in local.ext_vwan_dns_network_locations :
    location =>
    "${local.ext_private_dns_resolver_dns_forwarding_ruleset_prefix[location]}/${local.ext_private_dns_resolver_dns_forwarding_ruleset_name[location]}"
  }
  azurerm_ext_private_dns_resolver = [
    for location, ext_vwan_dns_config in local.ext_vwan_dns_networks_by_location :
    {
      # Resource logic attributes
      resource_id           = local.ext_private_dns_resolver_resource_id[location]
      managed_by_module     = local.deploy_ext_private_dns_resolver[location]
      # Resource definition attributes
      name                  = local.ext_private_dns_resolver_name[location]
      resource_group_name   = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
      location              = location
      virtual_network_id    = try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].virtual_network_id, local.ext_vwan_dns_network_resource_id[location])
      tags                  = try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].tags, local.tags)
    }
  ]
  azurerm_ext_private_dns_resolver_inbound_endpoint = [
    for location, ext_vwan_dns_config in local.ext_vwan_dns_networks_by_location :
    {
      # Resource logic attributes
      resource_id             = local.ext_private_dns_resolver_inbound_endpoint_resource_id[location]
      managed_by_module       = local.deploy_ext_private_dns_resolver_inbound_endpoint[location]
      # Resource definition attributes
      name                    = local.ext_private_dns_resolver_inbound_endpoint_name[location]
      resource_group_name     = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
      location                = location
      private_dns_resolver_id = local.ext_private_dns_resolver_resource_id[location]
      ip_configurations  = [{
        private_ip_allocation_method = "Dynamic"
        subnet_id                    = try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].inbound_subnet_id, "${local.ext_vwan_dns_network_resource_id[location]}/subnets/PrivateResolverIn")
      }]
      tags                    = try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].tags, local.tags)
    }
  ]
  azurerm_ext_private_dns_resolver_outbound_endpoint = [
    for location, ext_vwan_dns_config in local.ext_vwan_dns_networks_by_location :
    {
      # Resource logic attributes
      resource_id             = local.ext_private_dns_resolver_outbound_endpoint_resource_id[location]
      managed_by_module       = local.deploy_ext_private_dns_resolver_outbound_endpoint[location]
      # Resource definition attributes
      name                    = local.ext_private_dns_resolver_outbound_endpoint_name[location]
      resource_group_name     = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
      location                = location
      private_dns_resolver_id = local.ext_private_dns_resolver_resource_id[location]
      subnet_id               = try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].outbound_subnet_id, "${local.ext_vwan_dns_network_resource_id[location]}/subnets/PrivateResolverOut")
      tags                    = try(local.custom_settings.private_dns_resolver["extended_vwan_dns"][location].tags, local.tags)
    }
  ]
  azurerm_ext_private_dns_resolver_dns_forwarding_ruleset = [
    for location, ext_vwan_dns_config in local.ext_vwan_dns_networks_by_location :
    {
     # Resource logic attributes
      resource_id                                = local.ext_private_dns_resolver_dns_forwarding_ruleset_resource_id[location]
      managed_by_module                          = local.deploy_ext_private_dns_resolver_dns_forwarding_ruleset[location]
      # Resource definition attributes
      name                                       = local.ext_private_dns_resolver_dns_forwarding_ruleset_name[location]
      resource_group_name                        = local.resource_group_names_by_scope_and_location["extended_vwan_dns"][location]
      location                                   = location
      private_dns_resolver_outbound_endpoint_ids = [local.ext_private_dns_resolver_outbound_endpoint_resource_id[location]]
      tags                                       = try(local.custom_settings.ext_private_dns_resolver_dns_forwarding_ruleset["extended_vwan_dns"][location].tags, local.tags)
    }
  ]
  ext_private_dns_resolver_forwarding_rule = {
    for location, ext_vwan_dns_network in local.ext_vwan_dns_networks_by_location:

              location => concat (
                [
                  for forwarding_rule in ext_vwan_dns_network.config.private_dns_resolver_forwarding_rule : merge(
                    forwarding_rule,
                    {
                      # Resource logic attributes
                      resource_id = "${local.ext_vwan_dns_network_resource_group_id[location]}/providers/dnsForwardingRulesets/forwardingRules/${forwarding_rule.name}"
                      managed_by_module = local.deploy_ext_private_dns_resolver_dns_forwarding_ruleset[location]
                      scope             = "extended_vwan_dns"
                      # Resource definition attributes
                      name                      = forwarding_rule.name
                      dns_forwarding_ruleset_id = local.ext_private_dns_resolver_dns_forwarding_ruleset_resource_id[location]
                      domain_name               = forwarding_rule.domain_name
                      enabled                   = forwarding_rule.enabled
                      target_dns_servers        = forwarding_rule.target_dns_servers
                      metadata                  = forwarding_rule.metadata
...
#  - azurerm_private_dns_resolver_forwarding_rule
locals {
azurerm_ext_private_dns_resolver_forwarding_rule = flatten([
    for forwarding_rule in local.ext_private_dns_resolver_forwarding_rule:
    forwarding_rule
  ])
}
  ...
  module_output = {
    ...
    azurerm_subnet_network_security_group_association = [
      for resource in local.azurerm_subnet_network_security_group_association :
      {
        ...
        managed_by_module = resource.network_security_group_id != "" ? true : false
      }
    ]
    azurerm_subnet_route_table_association = [
      for resource in local.azurerm_subnet_route_table_association :
      {
        ...
        managed_by_module = resource.route_table_id != "" ? true : false
      }
    ]
    azurerm_bastion_host = [
    ...
    azurerm_virtual_network_bastion = [
      for resource in local.ext_vwan_bastion_network :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_vwan_bastion_subnet = [
      for resource in local.azurerm_ext_vwan_bastion_subnet :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_vwan_bastion_nsg = [
      for resource in local.azurerm_ext_vwan_bastion_nsg :
      {
        resource_id   = resource.resource_id
    ...
    azurerm_ext_vwan_bastion_network_security_group_association = [
      for resource in local.azurerm_ext_vwan_bastion_network_security_group_association :
      {
        resource_id   = resource.subnet_id
        template = {
          for key, value in resource :
    ...
    azurerm_ext_vwan_bastion_host = [
      for resource in local.azurerm_ext_vwan_bastion_host :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_virtual_network_dns = [
      for resource in local.ext_vwan_dns_network :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
          for key, value in resource :
          key => value
          if local.deploy_ext_vwan_dns_network[resource.location] &&
          key != "resource_id" &&
          key != "managed_by_module"
        }
        managed_by_module = local.deploy_ext_vwan_dns_network[resource.location]
      }
    ]
    azurerm_ext_vwan_dns_subnet = [
      for resource in local.azurerm_ext_vwan_dns_subnet :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_private_dns_resolver = [
      for resource in local.azurerm_ext_private_dns_resolver :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_private_dns_resolver_inbound_endpoint = [
      for resource in local.azurerm_ext_private_dns_resolver_inbound_endpoint :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_private_dns_resolver_outbound_endpoint = [
      for resource in local.azurerm_ext_private_dns_resolver_outbound_endpoint :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_private_dns_resolver_dns_forwarding_ruleset = [
      for resource in local.azurerm_ext_private_dns_resolver_dns_forwarding_ruleset :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
    ...
    azurerm_ext_private_dns_resolver_forwarding_rule = [
      for resource in local.azurerm_ext_private_dns_resolver_forwarding_rule :
      {
        resource_id   = resource.resource_id
        resource_name = resource.name
        template = {
